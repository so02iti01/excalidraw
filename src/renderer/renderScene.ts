...
const _renderInteractiveScene = ({
  ...
  // FIXME II: reuse canvas boostrap
...
const _renderStaticScene = ({
  ...
  if (isExporting && appState.theme === "dark") {
  ...
  if (typeof appState.viewBackgroundColor === "string") {
    const hasTransparence =
      ...
      appState.viewBackgroundColor.length === 5 || // #RGBA
      appState.viewBackgroundColor.length === 9 || // #RRGGBBA
      /(hsla|rgba)\(/.test(appState.viewBackgroundColor);
    if (hasTransparence) {
      context.clearRect(0, 0, normalizedCanvasWidth, normalizedCanvasHeight);
    }
    context.save();
  ...
  if (renderGrid && appState.gridSize) {
    strokeGrid(
      ...
      normalizedCanvasWidth / appState.zoom.value,
      normalizedCanvasHeight / appState.zoom.value,
    );
  }
